...
class TemporaryBusinessPermitPdf < Prawn::Document
  ...
  def heading
    ...
    move_down 25
    text "TEMPORARY MAYOR'S PERMIT", align: :center, size: 28, style: :bold
  ...
  def assessment_details
    bounding_box([375, 600], :width => 170) do
      ...
      if assessment_table_data.empty?
        ...
        move_down 20
        text "#{(@business_permit.date_approved + 29.days).strftime("%B %e, %Y")}", style: :bold, size: 10, align: :center
  ...
  def footer
    ...
    bounding_box([400, 10], width: 120) do
