module Monitoring
  class UnrenewedBusinessesPdf < Prawn::Document
    require 'prawn/table'
    TABLE_WIDTHS = [50, 160, 120, 100, 100 ]

    def initialize(barangay, businesses, from_date, to_date, view_context)
      super(page_size: 'LEGAL', margin: 30, page_layout: :landscape)
      @barangay = barangay
    ...
    def heading
      ...
      move_down 10
      if @barangay.present?
        text "LIST OF UNRENEWED BUSINESSES IN #{@barangay.name.upcase}", align: :center, size: 10, style: :bold
      else
        text "LIST OF UNRENEWED BUSINESSES IN LAMUT", align: :center, size: 10, style: :bold
      end
    ...
    def businesses_table
      if table_data.empty?
        text "No registered businesses found.", align: :center
      else
        table table_data,
          cell_style: { size: 8}
