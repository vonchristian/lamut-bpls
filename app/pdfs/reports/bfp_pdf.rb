  ...
  class BfpPdf < Prawn::Document
    ...
    def businesses_table
      if table_data.empty?
        ...
        table table_data,
        cell_style: { size: 8,  inline_format: true  }, column_widths: [60, 80, 50, 130, 100, 150] do
    ...
    def table_data
      move_down 5
      [["Date of Registration", "Taxpayer", "Business Type", "Business Name",  "Business Address", "Line of Business"]] +
      @table_data ||= @businesses.map { |e| [e.date_registered, e.taxpayers_full_name.try(:upcase), e.business_status, e.name.try(:upcase),  e.full_address, e.line_of_businesses_name] }
    end
