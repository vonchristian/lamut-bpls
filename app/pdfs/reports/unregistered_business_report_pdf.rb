module Reports
  class UnregisteredBusinessReportPdf < Prawn::Document
    require 'prawn/table'
    ...
    def heading
      ...
      move_down 10
      if @barangay.present?
        text "UNREGISTERED BUSINESSES IN #{@barangay.name.try(:upcase)}", align: :center, size: 10, style: :bold
      else
        text "UNREGISTERED BUSINESSES IN LAMUT", align: :center, size: 10, style: :bold
      end
      move_down 10
    end
    def logo
