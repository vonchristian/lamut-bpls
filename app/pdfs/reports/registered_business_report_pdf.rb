module Reports
  class RegisteredBusinessReportPdf < Prawn::Document
    require 'prawn/table'
    ...
    def heading
      ...
      move_down 10
      if @barangay.present?
        text "REGISTERED BUSINESSES IN #{@barangay.name.try(:upcase)}", align: :center, size: 10, style: :bold
      else
        text "REGISTERED BUSINESSES IN LAMUT", align: :center, size: 10, style: :bold
      end
      move_down 10
    end
    def logo
